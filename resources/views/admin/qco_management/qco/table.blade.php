        ...
        <div class="eve-list">
            <div class="inner @if($loop->last) inner-last @endif">
                ...
                <div class="inner-content">
                    <div class="image">
                        <img src="{{ asset('backend/admin/images/qco_management/qcos/' . $qco->images[0]) }}"
                    ...
                    <div class="content">
                        <div class="content-inner">
                            ...
                            <p>{{ $qco->short_description }}</p>

                            

                            <div class="dropdown status-dropdown">
                                <button type="button"
                                    class="@if ($qco->is_active == '1') status-active @else status-deactive @endif"
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuOffset{{ $key }}">
                                    <a class="toggleActiveBtn dropdown-item" href="javascript:void(0)" data-id="{{ $qco->id }}">
                                        Publish
                                    </a>
                                    <a class="toggleActiveBtn dropdown-item" href="javascript:void(0)" data-id="{{ $qco->id }}">
                                        Unpublished
                ...
                <div class="qco-action">

                    <div class="evt-action qco-date">
                        ...
                        @if ($qco->single_day == '1')
                            <span>{{ Carbon\Carbon::parse($qco->date)->format('d M Y') }}</span>
                        @else
                            <span>{{ Carbon\Carbon::parse($qco->start_date)->format('d M Y') }}</span>
                        @endif
                    </div>
                    ...
                    <div class="dropdown">
                        <div class="evt-action qco-more-details" id="self_car_{{ $key }}"
                        ...
                        <div class="dropdown-more-details menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4 mt-10 new-dp1 dropdown-menu"
                            ...
                            <div class="menu-item px-3">
                                ...
                                <a href="{{ route('admin.qcos.show', $qco->id) }}" class="menu-link px-3">
                                ...
                                @can('qco-edit')
                                    <a href="{{ route('admin.qcos.edit', $qco->id) }}" class="menu-link px-3">
                                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                    ...
                                    {{-- <form action="{{route('admin.qcos.destroy',$qco->id)}}" method="POST" id="delete-form">
                                @method('DELETE')
                                @csrf
                            <a href="#" class="menu-link px-3"  onclick="qco.prqcoDefault();document.getElementById('delete-form').submit();">
                                ...
                                @endcan
                                @can('qco-delete')
                                    <a class="menu-link px-3" onclick="confirmDelete({{ $qco->id }})">
                                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                    ...
                                    <form action="{{ route('admin.qcos.destroy', $qco->id) }}"
                                        id="delete_form_{{ $qco->id }}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                    </form>
                                @endcan
...
<div class="row">
    {{-- <div class="col-md-4">
        <p>
            <b>Showing {{ ($qcos->currentpage() - 1) * $qcos->perpage() + 1 }} to
                {{ ($qcos->currentpage() - 1) * $qcos->perpage() + $qcos->count() }} of
                {{ $qcos->total() }} qcos
            </b>
        </p>
    </div> --}}
...
<script>
    $(function() {
        $('.page-link').on('click', function(qco) {
            qco.prqcoDefault()
