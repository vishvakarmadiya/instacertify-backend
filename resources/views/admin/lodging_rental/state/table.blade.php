<table class="table align-middle table-row-dashed fs-6 gy-5">
    <thead>
        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
            ...
            @canany(['event-states-edit', 'event-states-delete'])
            ...
            <tr>
                ...
                <td>
                    @if ($state->is_active == '1')
                        <a href="{{ route('admin.states.show', $state->id) }}?status=0">
                            <div class="badge badge-light-success">Active</div>
                        </a>
                    @else
                        <a href="{{ route('admin.states.show', $state->id) }}?status=1">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-more-details menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4 mt-10 new-dp1 dropdown-menu"
                                ...
                                <div class="menu-item px-3">
                                    ...
                                    @endcan
                                    @can('states-delete')
                                        <a class="menu-link px-3" onclick="confirmDelete({{ $state->id }})">
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                        ...
                                        <form action="{{ route('admin.states.destroy', $state->id) }}"
                                            id="delete_form_{{ $state->id }}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                    @endcan
...
<div class="row">
    <div class="col-md-4">
        <p>
            <b>Showing {{ ($states->currentpage() - 1) * $states->perpage() + 1 }} to
