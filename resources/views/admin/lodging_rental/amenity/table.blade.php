<table class="table align-middle table-row-dashed fs-6 gy-5">
    <thead>
        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
            ...
            @canany(['event-amenities-edit', 'event-amenities-delete'])
    ...
    <tbody>
        @foreach ($amenities as $key => $amenity)
            <tr>
                <td>{{ $key + 1 + ($amenities->currentPage() - 1) * $amenities->perPage() }}</td>
                <td>
                    <div class="d-flex align-items-center">
                        <div class="symbol symbol-45px me-5">
                            <img src="{{ asset('backend/admin/images/lodging_rental/amenities/' . $amenity->image) }}"
                ...
                <td>
                    @if ($amenity->is_active == '1')
                        <a href="{{ route('admin.amenities.show', $amenity->id) }}?status=0">
                            <div class="badge badge-light-success">Active</div>
                        </a>
                    @else
                        <a href="{{ route('admin.amenities.show', $amenity->id) }}?status=1">
                            <div class="badge badge-light-danger">Deactive</div>
                        </a>
                    @endif
                </td>
                @canany(['amenities-edit', 'amenities-delete'])
                            ...
                            <div class="dropdown-more-details menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4 mt-10 new-dp1 dropdown-menu"
                                ...
                                <div class="menu-item px-3">
                                    ...
                                    @can('amenities-delete')
                                        <a class="menu-link px-3" onclick="confirmDelete({{ $amenity->id }})">
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                        ...
                                        <form action="{{ route('admin.amenities.destroy', $amenity->id) }}"
                                            id="delete_form_{{ $amenity->id }}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                    @endcan
...
<div class="row">
    <div class="col-md-4">
        <p>
            <b>Showing {{ ($amenities->currentpage() - 1) * $amenities->perpage() + 1 }} to
                {{ ($amenities->currentpage() - 1) * $amenities->perpage() + $amenities->count() }} of
