GoogleFontsManager = {
	...
	updateFontList: function () {
		let googleFontsLink = $("#google-fonts-link", web.Builder.frameHead);
		...
		if (googleFontsLink.length < 1) {
			...
			web.Builder.frameHead.append(googleFontsLink);
		}
...
web.FontsManager.addProvider("google", GoogleFontsManager);
	
let googleFonts = {};
let googlefontNames = [];
//load google fonts list and update wyswyg font selector and style tab font-family list
$.ajax({
	url: web.baseUrl + "../../resources/google-fonts.json",
	cache: true,
	dataType: "json",
	success: function(data) {
		...
		let list = web.Components.getProperty("_base", "font-family");
		if (list) {
			list.onChange = function (node,value, input, component) {
				let option = input.options[input.selectedIndex];
				web.FontsManager.addFont(option.dataset.provider, value, node);
				return node;
			};
			...
			web.Components.updateProperty("_base", "font-family", {data:list.data});
