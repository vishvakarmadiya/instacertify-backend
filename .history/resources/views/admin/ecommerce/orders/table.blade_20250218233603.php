...
$(document).ready(function() {
    // Check if DataTable is already initialized to avoid reinitialization errors
    if (!$.fn.dataTable.isDataTable('#kt_datatable_zero_configuration')) {
        $("#kt_datatable_zero_configuration").DataTable({
            "paging": true,             // Enable pagination
            "searching": false,        // Disable searching
            "info": true,              // Display table info (e.g., "Showing 1 to 10 of 100 entries")
            "lengthChange": true,      // Allow changing the number of records per page
            "pageLength": 10,          // Set the default records per page to 10
            "order": [[0, 'desc']],    // Sort by first column (ID) in descending order
            "pagingType": "simple_numbers", // Use "simple_numbers" for pagination controls (Previous/Next)
            "language": {
                "paginate": {
...
$(document).ready(function() {
    function loadTable(url) {
        $.ajax({
            ...
            success: function(data) {
                ...
                if ($.fn.DataTable.isDataTable("#kt_datatable_zero_configuration")) {
                    $("#kt_datatable_zero_configuration").DataTable().destroy();
                }
                $("#kt_datatable_zero_configuration").DataTable({
                    "paging": true,             
                    "searching": false,       
    ...
    loadTable("{{ route('admin.orders.index') }}");
});
