...
<table class="table align-middle table-row-dashed fs-6 gy-5 table-row-bordered" id="order_table">
    <thead>
...
<script>
  $(document).ready(function() {
    // Initialize DataTable with pagination, searching disabled, etc.
    initializeDataTable();
    
    // Open and close the modal on button click
...
function showOrderDetails(order) {
    const orderInfoContent = generateOrderInfoContent(order);
    const addressContent = generateAddressContent(order.address);
    const itemsContent = generateItemsContent(order.order_items);
...
function generateOrderInfoContent(order) {
    ...
    return `
        ...
        ${order.order_delevered_date ? `<tr><th>Delivered Date</th><td>${new Date(order.order_delevered_date).toLocaleString()}</td></tr>` : ''}
        ${order.order_cancelled_date ? `<tr><th>Cancelled Date</th><td>${new Date(order.order_cancelled_date).toLocaleString()}</td></tr>` : ''}
    `;
}

function generateAddressContent(address) {
    return `
...
function generateItemsContent(orderItems) {
...
function togglePopup() {
    // Code for opening/closing popup
    popup_open_close();
}
